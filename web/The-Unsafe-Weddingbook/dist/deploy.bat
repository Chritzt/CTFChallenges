...
docker build -t localhost/weddingbook --platform linux/amd64 .
goto :eof

:run
echo [+] Running Challenge Docker Container on 127.0.0.1:8080
docker run --name weddingbook-chall --rm -p 127.0.0.1:8080:8080 -t -i -e HOST=127.0.0.1 -e PORT=80 -e TIMEOUT=30  --read-only --privileged --platform linux/amd64 localhost/weddingbook
goto :eof

:kill
echo [-] Stopping Challenge Container
docker stop Weddingbook-chall >nul 2>&1
goto :eof
